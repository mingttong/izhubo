...
const crawlModel = require('../model/crawl');
const showTimeModel = require('../model/showTime');

const moment = require('moment');

const MAX_MSG_COUNT = 30;
const SHOW_STATUS_DELAY = 120 * 1000;
const CHECK_LOOP_DELAY = 60 * 1000;
  ...
  if (show_status_in_anchor_info === true && show_status_in_room_list === false) {
    ...
    if (Date.now() - roomInfo.start_show_time >= SHOW_STATUS_DELAY) {
      ...
      showTimeModel.create({
        ...
        owner_name,
        room_name,
        show_time_length: +((Date.now() / 1000 - moment(show_time).format('X')) / 60 / 60).toFixed(2),
