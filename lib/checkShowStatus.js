...
async function checkShowStatus(roomInfo) {
  ...
  if (anchorInfo.error_response) {
    console.log(anchorInfo.error_response.msg);
    return;
  }
  ...
  if (show_status_in_anchor_info === true && show_status_in_room_list === false) {
    ...
    if (Date.now() - roomInfo.end_show_time >= SHOW_STATUS_DELAY) {
      ...
      if (run_count > 1) {
         ...
         */
        sendMsg({
          sms_template_code: 'SMS_70225324',
          sms_param: {
            an: owner_name,       // 主播名
            rid: room_id,        // 房间号
            rn: room_name,        // 房间名
          ...
          rec_num: 18515220443,    // 用户的电话号码

        })
          .then(function (v) {
            msg_count++;
            logOutput(`Send message to 【管理员】 when 【${owner_name}】 show start`);
            console.log(v);
          })
          .catch(function (err) {
            logOutput(`ERROR when send message to 【管理员】`);
            console.log(err);
